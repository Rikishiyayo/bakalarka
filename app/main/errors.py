...
def page_not_found(err):
    current_app.logger.error('Page Not Found', exc_info=err)
...
def key_error(err):
    current_app.logger.error('Invalid request resulted in KeyError', exc_info=err)
    return render_template('error/error_page.html', message=""), 400


@main.app_errorhandler(500)
def internal_server_error(err):
    current_app.logger.error('Internal Server Error', exc_info=err)
