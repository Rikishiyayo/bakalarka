...
from config import APP_ROOT, EXP_DIRECTORY
...
def create_experiment(form, user_id):
    ...
    os.chdir(os.path.join(EXP_DIRECTORY, user_id))
    os.mkdir(exp_identificator)

    create_params_file(os.path.join(EXP_DIRECTORY, user_id, exp_identificator), form)
    create_result_file(os.path.join(EXP_DIRECTORY, user_id, exp_identificator))
    upload_file(form.models.data, os.path.join(EXP_DIRECTORY, user_id, exp_identificator))
    upload_file(form.expData.data, os.path.join(EXP_DIRECTORY, user_id, exp_identificator))
...
def get_model_data(user_id, exp_guid):
    src_file_path_models = os.path.join(EXP_DIRECTORY, user_id, exp_guid, "final.pdb")
    dst_file_path_models = os.path.join(APP_ROOT, "app/static/uploads/final.pdb")
    shutil.copyfile(src_file_path_models, dst_file_path_models)

    src_file_path_exp_data = os.path.join(EXP_DIRECTORY, user_id, exp_guid, "exp_data.dat")
    dst_file_path_exp_data = os.path.join(APP_ROOT, "app/static/uploads/exp_data.dat")
    shutil.copyfile(src_file_path_exp_data, dst_file_path_exp_data)


def create_user_directory(user_id):
    # create a directory for a new user
    os.chdir(os.path.join(EXP_DIRECTORY))
