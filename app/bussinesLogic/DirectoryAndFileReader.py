...
def get_experiments(user_id):
    ...
    for item in os.listdir(os.path.join(EXP_DIRECTORY, str(user_id))):
        info = { "exp_guid": item, "user_id": user_id }
        read_file(os.path.join(EXP_DIRECTORY, str(user_id), item, "params.txt"), info, ["title", "date"])
        read_file(os.path.join(EXP_DIRECTORY, str(user_id), item, "result.dat"), info, ["status", "progress"])
...
def get_computed_values_for_models(user_id, exp_guid):
    directory = os.path.join(EXP_DIRECTORY, user_id, exp_guid)
    models = []
...
def get_weights(user_id, exp_guid):
    filepath = os.path.join(EXP_DIRECTORY, user_id, exp_guid, "result.dat")
    weights = []    
    # Open the text file for reading
    file = open(filepath)    
    lines = file.readlines()
...
def get_experiment_result_data(user_id, exp_guid):
    result_file_path = os.path.join(EXP_DIRECTORY, user_id, exp_guid, "result.dat")
...
def get_experiment_parameters(user_id, exp_guid):
    parameters_file_path = os.path.join(EXP_DIRECTORY, user_id, exp_guid, "params.txt")
